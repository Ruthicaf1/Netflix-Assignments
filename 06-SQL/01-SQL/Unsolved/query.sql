...
select emp_fil.emp_no, emp_fil.first_name, emp_fil.last_name, emp_fil.title as cur_title from emp_fil join cur_emp on ((emp_fil.emp_no=cur_emp.emp_no) and (emp_fil.from_date=cur_emp.most_recent));
...
select cur_title, count (*) as employee_count from lat_title group by cur_title;


-- Count the total number of employees per department

with count_by_dept as(
select dept_no, count(emp_no) as num_of_employee_per_dept from dept_emp group by dept_no )
select dept_name, count_by_dept.num_of_employee_per_dept from departments join count_by_dept on (departments.dept_no=count_by_dept.dept_no);
